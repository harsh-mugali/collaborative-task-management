...
const SideMenu = ({ activeMenu }) => {
    ...
    return <div className="w-64 h-[calc(100vh-61px)] bg-white border-r border-gray-200/50 sticky top-[61px] z-20">
        <div className="flex flex-col items-center justify-center mb-7 pt-5">
            ...
            {user?.role === "admin" && (
                <div className="text-[10px] font-medium bg-primary px-3 py-0.5 text-white rounded mt-1 ">
                    Admin
                </div>
            )}
            ...
            {sideMenuData.map((item, index) => {
                return <button
                    ...
                    className={`w-full flex items-center gap-4 text-[15px] ${activeMenu == item.label
                        ? "text-primary bg-linear-to-r from-blue-50/40 to-blue-100/50  border-r-3 border"
                        : ""
                    }py-2 px-6 mb-3 cursor-pointer`}
